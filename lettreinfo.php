...
class EELV_newsletter{
	...
	add_action( 'phpmailer_init',array(&$this,'phpmailer_alter' ));
...
<?php
  ...
  *****************************************************************************************************************************************/
    ...
    ///////////////////////////////////// SEMI CRON AUTO SEND
    function phpmailer_alter(){
	global $phpmailer;
	if(!isset($phpmailer->Subject) || !$this->phpmailer_to_alter){
	    return;
	}
	$phpmailer->LE =$this->eol;
	foreach($this->phpmailer_to_alter as $attribute=>$value){
	    $phpmailer->$attribute=$value;
	}
	...
	foreach($IDS as $nl_id){
	    ...
	    // Prevent from phpmailer rewrite
	    //$this->phpmailer_to_alter['ReturnPath']=$reponse;
	    $this->phpmailer_to_alter['Sender']=$reponse;
	    $this->phpmailer_to_alter['From']=$reponse;
	    $this->phpmailer_to_alter['FromName']=$expediteur;
	    $this->phpmailer_to_alter['MessageID']='<'.$nl_id.'@'.$prov.'>';
	    $this->phpmailer_to_alter['XMailer']='EELV-Newsletter '.$this->pluginversion;
	    if($this->mime_type=='html_txt'){
		//$this->phpmailer_to_alter['ContentType']='multipart/alternative;';
		//$this->phpmailer_to_alter['Boundary']=array('',$boundary);
	    }
	    else{
		//$this->phpmailer_to_alter['ContentType']='text/html';
		//$this->phpmailer_to_alter['Charset']='UTF-8';
	    } // */
		...
		if ($destinataire && filter_var($dest, FILTER_VALIDATE_EMAIL)) {
			...
			$this->phpmailer_to_alter['Subject']=$the_sujet;
